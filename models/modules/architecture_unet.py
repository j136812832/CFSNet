...
class CFSNet(nn.Module):
    def __init__(self, in_channel, out_channel, num_channels, num_main_blocks, num_tuning_blocks, upscale=4,task_type='sr'):
        ...
        self.main = main_unet.MainNet(in_channel, out_channel, num_channels, num_main_blocks, task_type=task_type, upscale=upscale)
    ...
    def forward(self, x, control_vector):
        ...
        head_f = out
        out2 = torch.rand(1, 2, 3, 3)
        out3 = torch.rand(1, 2, 3, 3)
        out4 = torch.rand(1, 2, 3, 3)
        out5 = torch.rand(1, 2, 3, 3)
        for i, body in enumerate(self.main.body):
            tun_out, tun_alpha = self.tuning_blocks(x=out, alpha= control_vector.cuda(),number=i)
            # print("tun_alpha :{}".format(tun_alpha.shape))
            if (i == 0):
            ...
            if (i == 3):
                ...
                out5 = out
            if (i == 4):
                out = body(out5, out4) * tun_alpha + tun_out
            if (i == 5):
                out = body(out, out3) * tun_alpha + tun_out
            if (i == 6):
                out = body(out, out2) * tun_alpha + tun_out
            if (i == 7):
                out = body(out, head_f) * tun_alpha + tun_out
...
class Discriminator_VGG_128(nn.Module):
    def __init__(self, in_nc, nf):
        ...
        self.conv6_1 = nn.Conv2d(nf * 4, nf * 4, 3, 1, 1, bias=False)
